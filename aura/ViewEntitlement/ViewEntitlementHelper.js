({
    init : function(component, event, showHierarchyboolean) {
        ...
        action.setCallback(this, function(response) {
            ...
            //set tab label
            this.setFocusedTabLabel(component);
        });
    ...
    showEntitlementInformation:function(component,event,showHierarchy) {
        ...
        component.set('v.licenseInfoStartIndex',0);
        component.set('v.licenseInfoEndIndex',10);
        component.set('v.summaryStartIndex',0);
        component.set('v.summaryEndIndex',10);
        
        component.set('v.prodName',event.currentTarget.getAttribute('data-attriVal1'));
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                console.log("entitlementSummarySize: " + component.get('v.entitlementSummarySize'));
                component.set('v.showPrevEntitlementInfo',false);
                component.set('v.showProductSummary',true);
            }
            else if (state === "ERROR") {
                var errors = response.getError();
                if (errors) {
                    component.set("v.showErrorDiv",true);
                    if (errors[0] && errors[0].message) {
                        console.log("Error message: " + errors[0].message);
                    }
                } 
    ...
    nextEntitlementInfo:function(component,event) {
        ...
        if(startIndex > 1 && endIndex <=component.get('v.entireProductUsageSummaryList').length) {
            component.set('v.showPrevEntitlementInfo', true);
        }
        else {
            component.set('v.showPrevEntitlementInfo', false);
    ...
    prevEntitlementInfo:function(component,event) {
        ...
        if(component.get('v.summaryEndIndex') >=component.get('v.entireProductUsageSummaryList').length) {
            endIndex = component.get('v.summaryStartIndex');
            startIndex = endIndex-entitlementSummarySize;
        }
        else {
            var startIndex =component.get('v.summaryStartIndex')-entitlementSummarySize;
            var endIndex  =component.get('v.summaryEndIndex')-entitlementSummarySize;
